...
public class Segment {
    ...
    double calculateLeftDistance(double time, double time2) {
        double ldist = 0;
        for (double t = time; t <=time2; t += 1 / Trajectory.SAMPLE_RATE){
            ldist += Math.sqrt(Math.pow(this.calculateLeftSplinePositionX(t + 1 / Trajectory.SAMPLE_RATE) - this.calculateLeftSplinePositionX(t), 2)
                    + Math.pow(this.calculateLeftSplinePositionY(t + 1 / Trajectory.SAMPLE_RATE) - this.calculateLeftSplinePositionY(t), 2));// * 1/SAMPLE_RATE
        }
        return ldist;
    }
    double calculateRightDistance(double time, double time2) {
        ...
        for (double t = time; t<=time2; t += 1 / Trajectory.SAMPLE_RATE);
        {
            rdist += Math.sqrt(Math.pow(this.calculateRightSplinePositionX(t + 1 / Trajectory.SAMPLE_RATE) -this.calculateRightSplinePositionX(t), 2)
                    +Math.pow(this.calculateRightSplinePositionY(t + 1 / Trajectory.SAMPLE_RATE) - this.calculateRightSplinePositionY(t), 2));
    ...
    double calculateLeftDistanceTime(double time){
        double dist = Trajectory.maxvel * Trajectory.timedelta;
        return 0;
    }
