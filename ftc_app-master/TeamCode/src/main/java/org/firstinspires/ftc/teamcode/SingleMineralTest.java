...
public class SingleMineralTest extends CustomLinearOpMode {
            ...
            while (opModeIsActive()) {
                if (tfod != null) {
                    ...
                    if (updatedRecognitions != null) {
                        ...
                        telemetry.addData("# of Gold Cubes Detected", golds.size());
                        telemetry.addData("# of Silver Balls Detected", silvers.size());
                        telemetry.update();

                        //this if statement is important, probably will need to be changed
                        //(Y) for gold, (X) silver
                        if (golds.size() != 0 && gamepad1.y) {
                            trackMineral(updatedRecognitions.get(golds.get(0)));
                        } else if (silvers.size() != 0 && gamepad1.x) {
    ...
    private void trackMineral(Recognition recognition) {
        ...
        while (!done && !gamepad1.b && opModeIsActive()) {
            ...
            if (Xcheck && Acheck) {
                //TODO: grab mineral
                telemetry.addLine("Grabbing mineral");
                done = true;
            }
            telemetry.update();
        }
